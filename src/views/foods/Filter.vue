        ...
        <div class="container container-list lg:max-w-6xl mx-auto">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            <div class="relative w-full aspect-square cursor-pointer rounded-3xl" v-for="item in meals" v-bind:key="item.id">
              <router-link :to="{ name: 'foodsDetail', params: { cat: this.$route.params.cat, id: item.id }}">
...
export default {
  ...
  data() {
    return {
      ...
      errorState: '',
      prevRoute: 'foods',
      breadVal: this.$route.params.cat,
      pageDesc: 'Foods by Category',
      pageTitle: `${this.$route.params.cat} Meals`,
      meals: [],
    };
  },
  components: {
  ...
  async mounted() {
    await HTTP.get('api/json/v1/1/filter.php', { params: { c: this.$route.params.cat } }).then((response) => {
