        ...
        <div class="container container-list lg:max-w-6xl mx-auto">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
            <div class="relative w-full aspect-square cursor-pointer rounded-3xl" v-for="item in meals" v-bind:key="item.id">
              <router-link :to="{ name: 'ingredientsDetail', params: { ing: this.$route.params.title, id: item.id }}">
...
export default {
  ...
  data() {
    return {
      ...
      dataAvailable: true,
      errorState: '',
      prevRoute: 'ingredients',
      breadVal: this.getDetailUrls(this.$route.params.title),
      pageTitle: `${this.getDetailUrls(this.$route.params.title)} Meals`,
      meals: [],
    };
  },
  components: {
  ...
  async mounted() {
    await HTTP.get('api/json/v1/1/filter.php', { params: { i: this.getDetailUrls(this.$route.params.title) } }).then((response) => {
